...
export function Logo({ light = false }: { light?: boolean }) {
  return (
    <Link href="/" className="group flex items-center gap-3" aria-label={`${company.name} home`}>
      <span className="grid h-11 w-11 place-items-center bg-plum-900 font-display text-lg font-bold tracking-tight text-white">
        DS
      </span>
      <span className="leading-none">
        <span className={`block font-display text-xl font-bold tracking-tight ${light ? "text-white" : "text-ink"}`}>
