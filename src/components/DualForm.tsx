...
function HireForm({ onPlum }: { onPlum: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-3">
      ...
      <input className={field} placeholder="Role or skill set needed" aria-label="Role or skill set needed" />
...
function WorkForm({ onPlum }: { onPlum: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-3">
      ...
      <button type="submit" className={`w-full py-4 text-sm font-bold uppercase tracking-wide ${onPlum ? "bg-white text-plum-950 hover:bg-paper" : "bg-plum-900 text-white hover:bg-plum-700"}`}>
        Submit resume
      </button>
      <p className={`text-center text-xs ${onPlum ? "text-white/50" : "text-steel"}`}>
